...
@mcp_server.tool()
def count_en_String(text: str, characters_max: int, *, characters_min: int = 0) -> dict:
    """
    Counts the number of characters in a given English sentence.
    text: string to count(str)(required)
    characters_max: Upper limit of characters to count (int)(required)
    characters_min: lower limit of characters (int)(optional, default is characters_max-10)
    """
    ...
    try:
        ...
        is_below = count < characters_min
        message = (
            f"単語数が上限({characters_max})を超えています。現在の単語数: {count}"
            if is_exceed
            else f"単語数が上限内({characters_max})です。現在の単語数: {count}"
        )
        message += (
            f"\n単語数が下限({characters_min})を下回っています。"
            if is_below
            else f"\n単語数が下限({characters_min})を上回っています。"
        )
        payload = {
            "success": True,
            "message": message,
        ...
        return {
            "success": False,
            "message": f"単語数のカウント中にエラーが発生しました",
            "count": 0,
            "is_exceed": False,
            "is_below": False,
        }
